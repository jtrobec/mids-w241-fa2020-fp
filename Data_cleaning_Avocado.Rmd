...
# bcp_sum$id <- 1:length(dg)
# #selecting draft_number with posterior probabltiy greater than 0.2
# sel <- as.data.frame(bcp_sum[which(bcp_x$posterior.prob > 0.5), ])
# sel <- sel[sel$id>20 & sel$id <50,]
# sel <- sel[which.max(sel$Probability),]
# #get the id
# time_of_change <- time(dg)[sel$id]
# time_of_change[1]
...
where `c` and constant and `D`, `B`, `N`, and `J` are dummy varible with 1 and 0
...
mod_b2
coefficients(mod_b1)
coeftest(mod_b1, vcov = vcovHC(mod_b, type = "HC1"))
